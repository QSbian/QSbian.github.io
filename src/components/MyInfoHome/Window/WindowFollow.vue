...
// Mock 数据
const mockGoodsData = [
    { id: '2', name: '不想画画', description: 'oc变成邪恶人外了？dawdadwdadwadwd', price: '199.00', is_sold: '2', image: '/02_daring.ico', userimage: '/02_daring.ico' },
...
const handleScroll = () => {
    ...
    localStorage.setItem('SCROLL_FOLLOW_POS_KEY', scrollTop.toString());
};
...
const refreshData = async () => {
    try {
        ...
        localStorage.setItem('SCROLL_FOLLOW_POS_KEY', '0');
        isTouchingTop.value = false;
        pullDistanceTop.value = 0;
        console.log("数据刷新完成！");
    } catch (error) {
...
const loadMoreData = async () => {
    ...
    try {
        ...
        if (cardsGridRef.value) {
            ...
            localStorage.setItem('SCROLL_FOLLOW_POS_KEY', prevScrollTop.toString());
        }
    } catch (error) {
        page.value--;
...
onActivated(() => {
    ...
    const savedPos = Number(localStorage.getItem('SCROLL_FOLLOW_POS_KEY')) || 0;
