...
const switchTab = async (id: number) => {
    ...
    if (showOriginalImage.value) {
        ...
        localStorage.setItem('currentOpenIndex', id.toString());
    }
    else {
...
const openOriginalImage = (url: string) => {
    currentOpenIndex.value = currentIndex.value;
...
const contentTouchStart = (e: TouchEvent) => {
    contentStartY.value = e.touches[0].clientY;
    isDragging.value = true
};
...
const contentTouchMove = (e: TouchEvent) => {
    ...
    const diff = contentStartY.value - currentY

    // 限制移动范围：最多向上移动200px
    if (diff > 0 && infoPosition.value < 200) {
        infoPosition.value = Math.min(diff, 200)
    }
    // 向下移动恢复
    else if (diff < 0 && infoPosition.value > 0) {
        infoPosition.value = Math.max(infoPosition.value + diff / 20, 0)
    }
}
...
const imgTouchEnd = () => {
    ...
    if (Math.abs(diffY) < Math.abs(diffX) && Math.abs(diffX) > threshold) {
        if (!showOriginalImage.value) {
            ...
            if (diffX > 0 && currentOpenIndex.value > 0) {
                switchTab(currentOpenIndex.value - 1);
            } else if (diffX < 0 && currentOpenIndex.value < goods.value.image.length - 1) {
                switchTab(currentOpenIndex.value + 1);
            }
        }
    }
    ...
    <div>
        ...
        <div class="message-detail">
            ...
            <div v-else class="content-container">
                <div class="image-container" @touchstart="imgTouchStart" @touchmove="imgTouchMove"
                    @touchend="imgTouchEnd">
                    <div class="image-wrapper">
                        <img class="main-img" v-for="(img, idx) in goods.image" :key="idx" :src="img"
                            :alt="`商品图 ${idx + 1}`" :class="{ active: idx === currentIndex }"
                            v-show="idx === currentIndex" @click="openOriginalImage(img)" />
                    </div>
                ...
                <div class="info-container" :style="{ transform: `translateY(-${infoPosition}px)` }"
            ...
            <div class="original-image-modal" v-if="showOriginalImage" @click="closeOriginalImage">
                <div class="modal-content" @click.stop @touchstart="imgTouchStart" @touchmove="imgTouchMove"
                    ...
                    <img class="original-img" v-for="(img, idx) in goods.image" :key="idx" :src="img"
                        :alt="`商品图 ${idx + 1}`" :class="{ active: idx === currentOpenIndex }"
                        v-show="idx === currentOpenIndex" />
                </div>
            </div>
        </div>
    </div>
