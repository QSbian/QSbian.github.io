...
const drawerBarList = ref([
    { id: 0, text: '偏好推荐', icon: 'prefer', activeIcon: 'prefer-filled', color: '#8a8a8a', activeColor: '#3e7bfa', size: 20 },
    { id: 1, text: '最新入驻', icon: 'updated', activeIcon: 'updated-filled', color: '#8a8a8a', activeColor: '#3e7bfa', size: 20 },
]);
...
<template>
    <div class="tabBar" data-v-inspector="src/TabBar/MyInfoHome/WindowBar.vue:26:5">
        <div v-for="item in drawerBarList" :key="item.id" class="tab-item" :class="{ 'active': currentTab === item.id }"
            @click="switchTab(item.id)" data-v-inspector="src/TabBar/MyInfoHome/WindowBar.vue:27:9">
